...
> Исправление от 14.08.2026. Адрес `0xECA`, использованный первым probe как
> начало пикселей, фактически указывал на служебный marker `00`. В форматах
> `0x32E3`/`0x43E3` BGRA payload начинается со следующего байта, `+0x3D` от
> texture object. Поэтому первый crash доказывал повреждение framing marker, а
> не запрет изменения Alpha. RGB-only writer работал случайно правильно из-за
> прежнего сдвига и исправлен атомарно вместе с parser.
