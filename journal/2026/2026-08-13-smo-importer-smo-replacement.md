...
> Исправление от 14.08.2026. Нативный debugger trace опроверг вывод о запрете
> замены Alpha. Для `0x32E3`/`0x43E3` байт `texture + 0x3C` является marker `00`
> сериализатора, а BGRA-пиксели начинаются с `+0x3D`. Диагностический RGBA writer
> был сдвинут на один байт, записывал `FF` в marker и тем самым превращал число
> строк `0x00000100` в `0xFF000100`. Именно последующий выход `spDXTexture` за
> границу буфера вызвал crash; сама замена корректно адресованного Alpha этим
> тестом не была признана несовместимой.
