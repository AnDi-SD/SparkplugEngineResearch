...
Рабочее дерево использует общий `SmoViewer.Core` и модуль записи `SmoViewer.Editing`.
Для сборки нужны Windows x64, полный workspace и Visual Studio C++/CMake;
`SparkplugViewerNative.dll` собирается автоматически и нужна рядом с приложением.
В рабочем дереве идёт перенос ядер по правилам от 8 сентября; это не новый выпуск.
Workspace использует actual render-support occurrences общего SceneBuilder,
сохраняет материал/имя конкретного Model и ключ контейнера/слота. Если один Model
повторяется в нескольких слотах, каталог сохраняет все вхождения, а создание
редактируемого документа явно сообщает `REPEATED_RENDERABLE_AUTHORING`.
Текущая модель команд не адресует такие слоты независимо; молчаливое склеивание
удалено. Изменение этой authoring-модели отложено до отдельной работы над редактором.
