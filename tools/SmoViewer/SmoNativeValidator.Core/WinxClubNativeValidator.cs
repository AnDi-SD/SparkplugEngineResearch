...
public sealed class WinxClubNativeValidator
{
    ...
    private static void ValidateRequest(NativeValidationRequest request)
    {
        ...
        if (!request.AllowFileNameOnlyLogicalPath && !logicalPath.Contains('\\'))
        {
            throw new NativeAssetPathException(
                "LogicalGameAssetPath must include the game directory (for example Characters\\Troll\\Troll.smo); file-name-only matching requires an explicit opt-in.");
        }
