...
internal static class Win32Native
{
    ...
    internal static uint AllocateAndWriteMemory(
        IntPtr process,
        byte[] bytes,
        uint preferredAddress = 0)
    {
        ArgumentNullException.ThrowIfNull(bytes);
        if (bytes.Length == 0)
            throw new ArgumentException("Remote allocation cannot be empty.", nameof(bytes));

        IntPtr allocation = VirtualAllocEx(
            process,
            (IntPtr)(long)preferredAddress,
            (UIntPtr)(uint)bytes.Length,
            MemCommit | MemReserve,
            PageReadWrite);
        if (allocation == IntPtr.Zero && preferredAddress != 0)
        {
            // The preferred address keeps research traces stable, but another
            // executable build may already occupy it. Let Windows choose a
            // valid address rather than making the validator profile-dependent.
        ...
        if (allocation == IntPtr.Zero)
        {
            throw new Win32Exception(
                ...
                $"VirtualAllocEx({bytes.Length}) failed.");
        }
        ...
        if (address <= 0 || address > uint.MaxValue)
        {
            throw new InvalidDataException(
                $"Remote allocation 0x{address:X} is outside the 32-bit target address space.");
        }

        uint address32 = (uint)address;
        WriteMemory(process, address32, bytes, executable: false);
        return address32;
    }
