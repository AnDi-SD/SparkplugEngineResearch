...
public sealed record NativeValidationRequest
{
    ...
    /// <summary>
    /// Requires the requested native startLevel state to be both current and
    /// active with an idle transition queue before a contextual pass.
    /// </summary>
    public bool RequireSceneReady { get; init; }
    public bool IncludeBloomCheckpoints { get; init; }
    /// <summary>
    /// Optional case-insensitive text filter for selected native case-folded and
    /// exact string-comparison probes. Null disables every comparator breakpoint.
    /// </summary>
    public string? StringComparisonProbe { get; init; }
...
public sealed record NativeValidationReport
{
    ...
    /// <summary>
    /// Read-only Media tree selected beside the explicit executable. BuildAssetPath
    /// results are rebound to this root inside the owned child process.
    /// </summary>
