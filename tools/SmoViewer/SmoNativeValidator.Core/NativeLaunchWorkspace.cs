...
internal sealed class NativeLaunchWorkspace : IDisposable
{
    ...
    internal string MediaSourceDirectory { get; }
        ...
        int? startLevel)
    {
        ...
        string mediaSource = ResolveMediaSource(executable);
    ...
    private static string ResolveMediaSource(string executable)
    {
        string? executableDirectory = Path.GetDirectoryName(executable);
        if (string.IsNullOrWhiteSpace(executableDirectory))
            throw new DirectoryNotFoundException("The WinxClub.exe directory is unavailable.");
        string media = Path.GetFullPath(Path.Combine(executableDirectory, "Media"));
        if (!Directory.Exists(media))
        {
            throw new DirectoryNotFoundException(
                $"The Media directory was not found next to the selected WinxClub.exe: {media}");
        }
        if (!StagedAssetSession.IsAscii(media))
        {
            throw new NativeAssetPathException(
                "The Media path next to WinxClub.exe is not ASCII and cannot be passed to the ANSI game runtime.");
        }
        return media;
    }
