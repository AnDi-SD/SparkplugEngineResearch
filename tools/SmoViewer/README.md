...
Исходники входят в [SparkplugEngineResearch](../../README.md), каталог `tools/SmoViewer`.
[Релиз 0.5.0](https://github.com/AnDi-SD/SparkplugEngineResearch/releases/tag/smoviewer-v0.5.0)
доступен в общем репозитории. Для сборки клонируйте весь репозиторий.

Исследовательский просмотрщик файлов `.smo` из PC-версии Winx Club.

В [0.7.0](RELEASE_NOTES_0.7.0.md) ядро воспроизведения SAN напрямую вызывает
восстановленные C++-классы Sparkplug через DLL: загрузку, sampler, node controller,
обновление world transforms и skin-матрицы. SMO decode и OpenGL/WPF остаются
адаптерами. Сборка исходников требует полного workspace SparkplugEngineResearch
и Visual Studio C++/CMake; нативная DLL собирается автоматически. Для готового
пакета достаточно Windows x64 и .NET 8 Desktop Runtime.

В текущем рабочем дереве чтение FFPS header, FAT и object headers также передано
общей реконструкции. C# сохраняет исходные байты, диагностику и сведения для
инспектора; неизвестный класс можно исследовать без фиктивного runtime factory.
MeshBV/wxFaceData, PC/portable mesh buffers и vertex layout также читаются общими
восстановленными классами. Удалены C# угадывание E0 layout и проверки E1 planning
header, отсутствующие в оригинальном читателе. Остальные typed resource decoders,
ещё требуют переноса. PS2 packet header и bounds читаются оригинальными
serializer operations; opaque DMA payload не исполняется. Это промежуточный этап;
даже каталог без просмотра теперь требует общей DLL Windows x64.
Raw texture pixels и PC mip-записи также читает общий serializer; C# получает
исходные смещения и DTO. XRGB preview/export использует восстановленный codec
и непрозрачную alpha. DXT1/3/5 доступны как metadata, BGRA-preview для них ещё
не подключён. Создание одного embedded PC BGRA объекта и запись mip payload
также вызывают общий writer. PC source reader теперь исполняется в общих классах:
выбранная representation отделена от пропущенных полей и от числа созданных
runtime mips. Необычные читаемые формы не получают разрешение на запись через
старый путь редактора. PS2 native section также читает общий metadata inspector;
raw mip descriptors сохранены, их размеры не угадываются. Полные PS2 source/runtime
и legacy-common source-оболочка остаются границами. Исторический combined
TextureData Corpus profile приостановлен до разделения PC/PS2 evidence.
Другие typed writers ещё требуют переноса. Generic field encoder вызывает
общий writer и сохраняет поддерживаемую ширину заголовка. Запись ID31, настоящих
пустых полей и принудительного extended ID для малых номеров приостановлена:
оригинальный writer не даёт прежнего lossless-контракта редактора. Пустой payload
без явной size reservation остаётся сокращённым вызовом записи завершителя `00`.
Префикс object reference также читает общий `spSerializer`, включая ссылки
материалов. Ненулевой ID требует size word; прежняя интерпретация четырёх байтов
как «legacy nonnull reference» удалена. Raw inspection не загружает объект
и отдельно проверяет границы/SBOO, тогда как runtime использует FAT/cache.
Scalar-поля Node теперь также читает общий `spNodeSerializer`: quaternion
передаётся без нормализации, repeated fields используют исходную семантику.
DTO различает authored flag bytes и `EffectiveFlags` настоящего изолированного
Node. Это не замена полной загрузки отношений/наследников. Transform adapter
больше не ищет похожие поля в произвольном классе. Индекс ID кэшируется на
время жизни документа. Scene runtime загружает настоящие Node и наследников из
ResourceGraph, сохраняя их классы и virtual UpdateWorld. C# больше не создаёт
для них отдельный граф базовых Node и не подставляет inverse-bind pose.
Начальные и SAN palette matrices вычисляет общий Skin по реальным костям.
Снимок хранит упорядоченные ссылки RenderNode/StaticRenderObject/PartitionRenderable,
включая повторы. Это состав ресурсов, не результат visibility или draw order.
Для скалярного compatibility placement выбирается фактический support Model;
неоднозначное размещение требует явного выбора экземпляра. SceneBuilder читает
каждый фактический слот контейнера и сохраняет OccurrenceKey; общий меш
декодируется один раз. Файл с неподдержанным factory
остаётся доступен для metadata inspection, но не получает вымышленный runtime.
Редактор проверяет итоговое размещение общим runtime и отказывает в выдаче
результата, если прежний обратный расчёт не воспроизводит requested world PRS
(например, при неравномерном масштабе родителя).

Editing также предоставляет общий `SmoMeshDataWriter`: typed triangle input
записывается исходными CPU buffer/MeshData serializers через DLL. Его используют
замена mesh resource, skinned splitting и skeleton carrier импортёра/level tools.
Portable/PC запись подтверждена оригиналом; UInt16 и16MiB — ограничения host.
Создание geometry-only MeshBV для новых collision branches также вызывает
исходный serializer через `SmoMeshBoundingVolumeWriter`; face data этот путь
не генерирует, collision queries в API не входят.
CollisionInfo Group/PRS также читает исходный serializer: quaternion сохраняется
как записан, включая неединичный/нулевой; native defaults и authored presence
разделены. Матрицу stored PRS строит общий affine. Runtime attachment/registration
и выбор окончательного collision placement ещё требуют отдельного переноса.
Матрицы StaticRenderObject теперь читает общий `spStaticRenderObjectSerializer`:
они независимы, отсутствующие поля сохраняют identity, повторные используют
последнее значение. Порядок полей, affine shape и связь inverse/world не
навязываются читателем. C# сохраняет список metadata-ссылок Model/Skin; полный
native reader использует семейство Renderable и общую owning-реализацию support
с RenderNode. StaticRenderObject зарегистрирован в общем resource graph;
общий загрузчик уже читает пространственные ресурсы Alfea01/02. Это ещё не
инициализация Scene/visibility. Старый алгоритм создания inverse
перемещён без изменения в `SmoPlacementTransformWriter.CreateLegacyStaticInverseTransform`:
это отложенная редакторская политика, не восстановленный метод StaticRenderObject.

Материалы инспектируются через общий `spMaterialSerializer` на настоящих
DXMaterial/pass/StdLayer/MaterialTexture. C# больше не повторяет порядок полей,
чтение состояний, цветов и UV; metadata-ссылки не заменяются подставными игровыми
объектами. Исходные правила повторных полей и NULL контроллеров сохраняются.
Старый DTO просмотра пока допускает один стандартный слой на проход и требует
инициализированный authored color/power; это ограничения адаптера, общий native
материал сохраняет до8×8 слоёв. Не поддерживаемый состав отклоняется явно.

Model/Skin читаются общими `spRenderableSerializer`/`spModelSerializer`/
`spSkinSerializer`. C# связывает ID с каталогом, без своих section/palette
парсеров и без нормализации inverse-bind матриц. Исходные повторы/очистка палитры
и optional inherited поля сохраняются. Native reader допускает отсутствие mesh;
текущий DTO потребителя требует bound MeshData и сообщает этот предел явно.

`spAnimTexController` читает общий serializer; выбор кадра вызывает исходный
`spTextureTrack` через сохранённый native handle. Повторные временные границы,
NULL-ссылки и исходные значения времени доступны инспектору без C# parser.
Время этого API — уже вычисленная позиция трека. Циклические часы ещё требуют
подключения к renderer. Старые эвристики texture binding удалены;
равномерная последовательность по именам не создаётся.

Поля UV и material-color inspectors читаются исходными TransFunctionEval,
ColorFuncEval и FunctionEval serializers. C# DTO копируют их значения без
своих defaults, nested section loops или запретов повторных полей. Это чтение
метаданных; material controller update и renderer binding не подразумеваются.

MaterialData, diffuse-color и render-state views используют один результат
общего reader, кешируемый на неизменяемом документе. Повторные state fields и
чёрный authored diffuse сохраняются. Pass metadata доступны и без texture layer;
ограничение прежнего полного DTO одним слоем не ограничивает эти скалярные views.
Скалярные views выбирают материал по настоящей ссылке Model/Skin.
Texture binding и материал существующих scene instances теперь приходят из
загруженного native resource graph, включая реальную cache identity.

`SmoRenderableCatalog` индексирует результат общих Model/Skin readers один раз
на документ. Он сохраняет отдельные material/mesh links каждого renderable,
включая NULL, и сообщает ошибки декодирования. Это каталог metadata; физический
owner используется только для совместимого вида сохранённого MeshData.
SharedMeshInstance.MaterialObjectIndex приходит из исходной Renderable-ссылки.

`SmoLoadedResources` копирует read-only снимок настоящих Model/Skin, материалов,
всех passes/layers и texture controllers. Исходный граф освобождается после
копирования; снимок кешируется на неизменяемом документе. BGRA берётся из
инициализированной CPU texture, выбранной оригинальным loader, без предпочтения
другой serialized representation. Сейчас upload adapter поддерживает runtime
BGRA/BGRX; другие форматы сообщаются явно. Каждый существующий экземпляр меша
получает свой материал, цвет и текстуру; Model не наследует Skin palette другого
потребителя. Значение diffuse=0 сохраняется.

Сцена просмотра перечисляет actual support membership: Alfea01 — 1141 Model,
Alfea02 — 1008 Model и отдельная диагностика ParticleSystem. PC menu сохраняет
207 ссылок на 110 Model, включая повторы. Это полный состав поддержанных
ресурсов выбранных файлов, не видимость или draw order игры. Выбор по имени
`trail_mesh` удалён. Skin input world берётся из его общего render path;
world контейнера не применяется второй раз поверх world-space bone palette.
Ключ геометрии frontend включает слот; дерево по-прежнему показывает реальные
объекты файла. Политики alpha preview и полный renderer runtime ещё мигрируются.

Старый texture resolver с подбором по именам/соседству и равномерной анимацией
удалён. Полный material snapshot доступен ядру и Scene DTO; текущий frontend
показывает один pass/один layer. Многопроходные материалы, UV и точная анимация
сохранены с явной диагностикой незавершённого подключения; вместо них не
генерируется прежняя имитация. NULL Material требует renderer default/current
state. Visibility/culling и точное исполнение renderer состояний остаются
дальнейшими этапами, готовность всего Viewer не заявляется.

`SparkplugSceneRuntime.Materials` использует тот же живой ResourceGraph для
накопления времени реальными RenderController, обновления material pass и
получения texture/UV после него. Вызовы и порядок задаёт приложение явно;
это ещё не весь AnimationManager/игровой кадр. Старые равномерные texture clocks
удалены из OpenGL/WPF обвязки. Автоматическое подключение нового material runtime
к frontend остаётся следующим этапом. Сам MaterialColorController пока не
загружается полной graph factory; его методы не заменены имитацией.

В 0.6.1 анимация переиспользует геометрию костей и обновляет только transforms.
Невидимая WPF-копия GPU-мешей сохраняет актуальные позиции для выбора мышью,
но больше не пересчитывает normals, уже обрабатываемые GPU. На Icy и Knut
медиана CPU pose handler в Release уменьшилась с 2,59/2,15 до 0,83/0,61 мс
(240 кадров на случай), выделение памяти — на 62–70%. Это замер обработчика,
не общего FPS. Проверены позы, геометрия маркеров, слои и ray picking;
выбор кости другой загруженной модели больше не заимствует позицию активной анимации.
...
- `SmoViewer.Core` содержит оставшиеся typed C# decoders и адаптер сцены; FFPS/FAT,
  object/field headers, MeshBV/face data, PC/portable mesh buffers, vertex layout,
  SAN, локальные матрицы узлов и skin-матрицы получает
  из общего C++ Sparkplug;
- `SmoViewer.Sparkplug` управляет C ABI/SafeHandle и вызывает C++-ядро Sparkplug;
- `SmoViewer.Editing` содержит операции записи и редактирования для других
  инструментов; Viewer на этот модуль не ссылается. Пространство имён прежних
  общих API сохранено, их код существует в одном экземпляре;
- `SmoViewer.Corpus` хранит инкрементальный SQLite-индекс файлов, объектов,
  прямых полей и будущих подвидов;
- `SmoViewer.Rendering.Wpf` владеет общим OpenGL 3.3 backend, render-input и
  viewport-математикой;
- `SmoViewer` остаётся GUI-host: создаёт GL-контекст, управляет камерой,
  selection и невидимой hit-test geometry, но не содержит реализацию GPU renderer.

`SmoViewer.Rendering.Wpf` предназначен также для SmoLVLcreator. Общая подготовка
сцены находится в `SmoViewer.Scene`; GUI добавляет выбор, слои и вспомогательные
объекты. Проверка кода от 2026-09-08 удаляет второй SAN sampler и объединяет
общие операции. Полная замена SMO-декодеров и старых эвристик ещё не завершена;
случаи и предложения зафиксированы в workspace-досье
`docs/research/viewer-core-audit-2026-09-08.md`. Проверочные сборки не являются релизом.
...
V5-база разделяет serializer version, неизвестный `0x08`, FFPS platform mask,
платформу корпуса, уникальный ресурс и его
физические PCK-вхождения, а также содержит scope для вариантов и семантики полей,
межплатформенные пары и evidence. Полные payload остаются только в исходных
игровых файлах; SQLite хранит hashes, метаданные и ограниченные значения.
Описание SMO-слоя, фактическая статистика и воспроизводимые команды находятся в
[`docs/research/smo-corpus-database.md`](../../docs/research/smo-corpus-database.md),
а общий каталог форматов, executable и зависимостей — в
[`docs/research/game-resource-database.md`](../../docs/research/game-resource-database.md).
Специализированные analyzers для `spMaterialColorController`, `spFog`,
`spOBBBV`, `spBoxBV`, `spSphereBV`, `spUVController`, `spLightData`, `spNode`,
`spRenderNode`, `spTextureData`, `spMaterialData`, `spMeshData`, `spModel`,
`spStaticRenderObject`, `spSkin`, `spCollisionInfo`, `spPartitionRenderable`,
`spPartitionNode`, `spOctreeNode`, `spBSPNode`, `spOcclusionVolume`, `spPartitionSystem`, `spZone`,
`spZonePortal` и `spZonePortalNode`
проверяют структуру по всему корпусу и обоим executable. Node analyzer декодирует
девять полей, transform defaults и обе формы child relationship; Fog analyzer
подтверждает общий PC/PS2 layout, два наблюдаемых type-варианта и точное
совпадение всех 314 одноимённых межплатформенных payload. OBB analyzer проверяет
423 объекта, три возможных поля и точное совпадение всех 87 PC/PS2-пар ресурсов;
Box BV analyzer проверяет оба поля, все шесть объектов и точное совпадение всех
2/2 PC и 2/2 PC/PS2-пар ресурсов;
Sphere BV analyzer проверяет position/radius, три уникальных объекта и точное
совпадение единственных PC и PC/PS2-пар;
UV analyzer перечитывает полные payload из SMO/PCK, декодирует семь evaluator,
UV pivot и rotation axis во всех 4 705 объектах и отличает разреженные default-формы
от настоящих подтипов. Light analyzer декодирует девять optional-полей, применяет
общие defaults и различает directional, point, spot и ambient; spot подтверждён
обоими executable, но в корпусе не встречается. Подтверждённые значения
отображаются в read-only инспекторе полей. Material analyzer строго проверяет
105 588 объектов: один–три прохода, legacy/current state blocks, цвета, static
UV и texture/controller relationships.
Mesh analyzer перечитывает 66 191 объекта и 87 330 полей, различает семь
cross-platform/Direct3D/native-PS2 вариантов, проверяет PC buffers, PS2 header,
DMA boundary и AABB. В частности, он не принимает платформу корпуса за тип
представления: пять `_ps2.smo` PC-корпуса действительно содержат 629 native PS2
mesh. Полный отчёт: [`smo-class-sp-mesh-data.md`](../../docs/research/smo-class-sp-mesh-data.md).
Model analyzer перечитывает 118 720 объектов и 705 928 полей, разделяет
унаследованный `spRenderable` и собственный serializer, проверяет
material/fog/base-mesh targets и семь вариантов присутствия. Полный отчёт:
[`smo-class-sp-model.md`](../../docs/research/smo-class-sp-model.md).
Static-render-object analyzer перечитывает 61 851 размещение, проверяет обе
матрицы и inline `spModel`, аннотирует 185 553 поля и отличает engine
transpose-basis `InvTransform` от математического inverse. Полный отчёт:
[`smo-class-sp-static-render-object.md`](../../docs/research/smo-class-sp-static-render-object.md).
Collision-info analyzer перечитывает 10 594 объекта, проверяет три исторические
формы Primitive/Group/Transform, четыре типа bounding volume target и старые
ID-only связи. Read-only Inspector показывает разрешённый primitive, числовую
group и разложенный transform. Полный отчёт:
[`smo-class-sp-collision-info.md`](../../docs/research/smo-class-sp-collision-info.md).
