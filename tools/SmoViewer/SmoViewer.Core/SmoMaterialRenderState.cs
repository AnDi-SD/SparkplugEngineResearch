...
public static class SmoMaterialRenderState
{
        ...
        SmoDocument document)
    {
        ...
        foreach (List<SmoObjectEntry> siblings in children.Values)
        {
            ...
            for (int meshIndex = 0; meshIndex < meshes.Length; meshIndex++)
            {
                    ...
                    : materials.Length == meshes.Length
                        ...
                        : Array.FindLastIndex(
                            ...
                            material => material.LogicalOffset <
                                        meshes[meshIndex].LogicalOffset);
                if (materialIndex < 0 ||
                    !TryDecode(
                        document,
                ...
                result[meshes[meshIndex].Index] = state;
            }
        }
        ...
        out SmoMaterialRenderStateInfo? state)
    {
        ...
        uint? operation = null;
        uint[]? materialRenderStates = null;
        int offset = ObjectSignatureSize;
        while (SmoDataBlockReader.TryReadHeader(
                   data, offset, out SmoDataBlockHeader header))
        {
            if (materialRenderStates is null &&
                header.FieldType == 0 &&
                header.PayloadSize == MaterialRenderStateCount * sizeof(uint))
            {
                materialRenderStates = new uint[MaterialRenderStateCount];
                for (int index = 0; index < materialRenderStates.Length; index++)
                {
                    materialRenderStates[index] =
                        BinaryPrimitives.ReadUInt32LittleEndian(
                            data[(header.PayloadOffset + index * sizeof(uint))..]);
                }
            }
            else if (!operation.HasValue &&
                     header.FieldType == 3 &&
                     header.PayloadSize == sizeof(uint))
            {
                operation = BinaryPrimitives.ReadUInt32LittleEndian(
                    data[header.PayloadOffset..]);
            }

            if (operation.HasValue && materialRenderStates is not null)
                break;
        ...
        state = Classify(operation.Value, materialRenderStates ?? []);
        return true;
    }
