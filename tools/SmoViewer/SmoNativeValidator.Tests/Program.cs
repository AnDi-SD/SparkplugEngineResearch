...
{
    NativeValidationRequest legacy = new()
    {
        ExecutablePath = @"D:\Winx\WinxClub.exe",
        AssetPath = @"D:\Models\Troll.smo",
        LogicalGameAssetPath = @"Characters\Troll\Troll.smo"
    };
    Equal(NativeValidationRoute.Contextual, legacy.Route,
        "legacy requests retain contextual routing by default");
    True(legacy.UseIsolatedLaunchWorkspace,
        "all native validation requests default to an isolated windowed workspace");
    Equal(0, (int)NativeValidationRoute.Contextual,
        "contextual route keeps the binary/default enum value");
    Equal(1, (int)NativeValidationRoute.FastGeneric,
        "fast route is an explicit opt-in enum value for legacy callers");
    ResolvedNativeValidationRoute legacyRoute = NativeValidationRouting.Resolve(legacy);
    Equal(@"Characters\Troll\Troll.smo", legacyRoute.TriggerGameAssetPath,
        "contextual route triggers on the original logical slot");
    Equal<int?>(null, legacyRoute.RequestedStartLevel,
        "legacy contextual route does not inject a start level");

    NativeValidationRequest fast = legacy with
    {
    ...
    ResolvedNativeValidationRoute zeroStart = NativeValidationRouting.Resolve(legacy with
    {
        StartLevel = 0
    });
    ...
    ResolvedNativeValidationRoute accelerated = NativeValidationRouting.Resolve(legacy with
    {
        UseIsolatedLaunchWorkspace = true,
    ...
    Throws<NativeAssetPathException>(() => NativeValidationRouting.Resolve(legacy with
    {
        TriggerGameAssetPath = @"Characters\Bloom\bloom.smo"
    }), "contextual trigger must remain the original logical slot");
    Throws<ArgumentException>(() => NativeValidationRouting.Resolve(legacy with
    {
    ...
    Throws<ArgumentOutOfRangeException>(() => NativeValidationRouting.Resolve(legacy with
    {
        Route = (NativeValidationRoute)99
    }), "undefined route values are rejected");

    ResolvedNativeValidationRoute legacyFileName = NativeValidationRouting.Resolve(legacy with
    {
        LogicalGameAssetPath = "Troll.smo",
        AllowFileNameOnlyLogicalPath = true
    });
    Equal("Troll.smo", legacyFileName.TriggerGameAssetPath,
        "legacy contextual file-name-only matching remains available by explicit opt-in");
}
...
void TestTracker()
{
    ...
    Equal(7, tracker.Events.Count, "tracker retains full ordered event stream");
    ...
    Equal(Event(7, NativeValidationEventKind.Diagnostic).TimestampUtc,
