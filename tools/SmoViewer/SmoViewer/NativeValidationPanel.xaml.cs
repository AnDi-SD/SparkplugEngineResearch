...
public partial class NativeValidationPanel : System.Windows.Controls.UserControl, IDisposable
{
    ...
    public event EventHandler<HairPatcherLaunchRequestedEventArgs>? LaunchHairPatcherRequested;

    private void LaunchHairPatcherButton_Click(object sender, RoutedEventArgs e) =>
        LaunchHairPatcherRequested?.Invoke(
            this,
            new HairPatcherLaunchRequestedEventArgs(_gamePath));
    ...
    {
        LaunchHairPatcherButton.IsEnabled = enabled;
    ...
    private void UpdateStartLevelHint()
    {
        ...
        {
            StartLevelHintText.Text =
                "⚠ 50 — Gardenia04: в PC есть SPL/SPT, но отсутствует SMO уровня; " +
                "в PS2 отсутствует весь triplet. " +
...
public sealed class HairPatcherLaunchRequestedEventArgs(string? gameExecutablePath) : EventArgs
{
    public string? GameExecutablePath { get; } = gameExecutablePath;
}
