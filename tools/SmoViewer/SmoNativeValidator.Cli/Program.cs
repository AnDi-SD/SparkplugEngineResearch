...
internal static class Program
{
        ...
        for (int index = 0; index < cases.Count; index++)
        {
            ...
            try
            {
                NativeValidationRequest request = new()
                {
                    ...
                    AllowFileNameOnlyLogicalPath = testCase.AllowFileNameOnlyLogicalPath ?? options.AllowFileNameOnlyLogicalPath
                };
    ...
    private static string BuildTsv(IReadOnlyList<CaseSummary> summaries)
    {
        ...
        result.AppendLine(
            ...
            "ffps_header\tffps_magic\tffps_version\texception_code\texception_eip\t" +
            "exit_code\tredirect_count\tlog\tsummary\troute\ttrigger\tapplied_start_level\t" +
            "isolated\treport_duration_ms\tcrash_phase\tcrash_attribution_confidence");
...
Параметры:
  ...
  --allow-file-name-only          Разрешить logical только с именем файла.
...
Манифест может переопределять route, trigger, startLevel и useIsolatedLaunchWorkspace для кейса.
""");
    }
    ...
    private sealed class CliOptions
    {
        ...
        public bool AllowFileNameOnlyLogicalPath { get; private set; }
            ...
            for (int index = 0; index < args.Length; index++)
            {
                ...
                switch (argument.ToLowerInvariant())
                {
                    ...
                    case "--no-stage":
                        ...
                        break;
                    case "--allow-file-name-only":
                        result.AllowFileNameOnlyLogicalPath = true;
                        break;
                    case "--verbose":
    ...
    private sealed class ValidationCase
    {
        ...
        public bool? AllowFileNameOnlyLogicalPath { get; set; }
    }
