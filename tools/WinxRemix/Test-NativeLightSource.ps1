...
cl /I"$dependencyInclude" /nologo /std:c++17 /EHsc /MT /O2 /W4 /I"$apiInclude" "$snapshot\test_native_light_source.cpp" /Fe:test_native_light_source.exe /link user32.lib
if errorlevel 1 exit /b %errorlevel%
cl /I"$dependencyInclude" /nologo /std:c++17 /EHsc /MT /O2 /W4 /I"$apiInclude" "$snapshot\test_scene_lights.cpp" /Fe:test_scene_lights.exe /link user32.lib
exit /b %errorlevel%
"@
