...
$build=Join-Path $root "local-data/rtx-remix/skin-packet-tests/$Name"
...
$source=Join-Path $build 'source/tools/WinxRemix/test_skin_packets.cpp'
$commands=@"
@echo off
call "$environmentScript" $Platform
if errorlevel 1 exit /b %errorlevel%
cl /nologo /std:c++17 /EHsc /MT /O2 /W4 "$source" /Fe:test_skin_packets.exe
exit /b %errorlevel%
"@
...
try {
    ...
    $process.StartInfo.FileName=Join-Path $build 'test_skin_packets.exe'
    $process.StartInfo.WorkingDirectory=$build
    $process.StartInfo.UseShellExecute=$false
    $process.StartInfo.CreateNoWindow=$true
    $process.StartInfo.RedirectStandardOutput=$true
    $process.StartInfo.RedirectStandardError=$true
    if (-not $process.Start()) { throw 'Cannot start packet tests' }
    ...
    $process.Dispose()
    if ($limited -or $exitCode -ne 0) { throw "Packet tests failed; see $build" }
