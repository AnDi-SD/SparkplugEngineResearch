...
$build=Join-Path $root "local-data/rtx-remix/test-scene-geometry/$Name"
...
$source=Join-Path $PSScriptRoot 'test_scene_geometry.cpp'
...
cl /I"$dependencyInclude" /nologo /std:c++17 /EHsc /MT /O2 /W4 /I"$include" "$source" /Fe:test_scene_geometry.exe /link user32.lib
if errorlevel 1 exit /b %errorlevel%
test_scene_geometry.exe
exit /b %errorlevel%
"@
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw "Scene geometry contract tests failed: $LASTEXITCODE" }
