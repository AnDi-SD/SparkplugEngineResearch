...
// Positive and signed B2 rotations use one immutable mesh each. A separate
// rigid API instance provides a simultaneous screen-space motion-vector oracle.
static unsigned MotionRotation(const SkinVertices& baseInput,const std::vector<uint32_t>& triangleIndices,
  ...
  // palette rotation on the right, equivalent rigid transform on the left.
  // Pixel samples compare the same frame, avoiding a temporal screenshot oracle.
  Config("rtx.postfx.enableMotionBlur","False");
  fprintf(journal,"{\"event\":\"rotation_contract\",\"vertices\":%zu,\"subdivisions\":%u,\"separationPixels\":288,\"skinCategoryIgnoresMotion\":false,\"anglesRadians\":[0.004,0.007]}\n",baseInput.size(),24u);fflush(journal);
  ...
  for(unsigned signedProfile=0;signedProfile<2;++signedProfile){
    ...
    const auto skin=Mesh(baseInput,triangleIndices,materialHandle,0x5758524f54411000ull+signedProfile,encoded.influences,encoded.weights,encoded.indices);
    const auto reference=Mesh(baseInput,triangleIndices,materialHandle,0x5758524f54412000ull+signedProfile,0,weights,bones);
    for(unsigned phase=0;phase<5;++phase){
      ...
      do {
        ...
        angle=(phase==2?.004f:phase==3?.007f:0.f)*sign;
        const std::array<remixapi_Transform,4> palette={Bone(angle,0,0),Bone(angle,0,0),Bone(angle,0,0),Bone(angle,0,0)};
        ...
        instance.pNext=&blend;instance.mesh=reference;instance.transform=Bone(angle,-1.5f,-.2f);
      ...
      char filename[80]{};sprintf_s(filename,"rotation-s%u-p%u.bmp",signedProfile,phase);Capture(filename);++captures;
      fprintf(journal,"{\"event\":\"rotation_phase\",\"signedProfile\":%u,\"phase\":%u,\"frames\":%u,\"angle\":%.9g,\"file\":\"%s\"}\n",signedProfile,phase,phaseFrames,angle,filename);fflush(journal);
    }
