...
Закреплённый renderer обновляет каждый тип камеры только один раз за кадр.
Поэтому `SetupCamera` должен попасть в очередь до первого draw, который может
установить тот же тип камеры. Поздний API-вызов возвращает `SUCCESS`, но
`RtCamera::update` оставляет первое состояние. Это проверяется
[сценарием Camera](../docs/skinning-fixture.md#история-движения) по экранным
векторам движения при переносе и повороте камеры. Игровой адаптер отдельно
проверяет окно первой подачи; транспортный ответ сам по себе этого не доказывает.
...
`Run-Live.py` использует общий собственный `winx_owned_process.py`; закрытый
каталог исследования для запуска не нужен. `--memory-limit-mib` задаёт предел
commit собственного job, `--reserve-mib` сохраняет доступную системную память
(по умолчанию 4096 и 512 МиБ). Лимит выбирают по текущим ресурсам. При нехватке
запаса прекращается только принадлежащий стенду job; такой результат не PASS.
