...
$build=Join-Path $root "local-data/rtx-remix/independent-submit-tests/$Name"
...
$source=Join-Path $snapshot 'test_independent_submit.cpp'
# Ordinary CPU fixture executable on owned records; no original game functions.
$commands=@"
@echo off
call "$environmentScript" x86
if errorlevel 1 exit /b %errorlevel%
cl /I"$dependencyInclude" /nologo /std:c++17 /EHsc /MT /O2 /W4 /I"$include" "$source" /Fe:test_independent_submit.exe /link user32.lib
exit /b %errorlevel%
"@
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw "Independent submit build failed; see $build/build.log" }
    if ($SkipRun) { Write-Output "Built CPU independent submit fixture: $build"; return }
    $exe=Join-Path $build 'test_independent_submit.exe'
    ...
    $process.Dispose()
    if ($exitCode -ne 0) { throw "Independent submit fixture failed ($exitCode); see $build/stderr.log" }
    $result=Get-Content -LiteralPath (Join-Path $build 'result.json') -Raw | ConvertFrom-Json
    if ($result.status -ne 'PASS') { throw 'Independent submit fixture did not report PASS' }
