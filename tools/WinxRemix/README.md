...
Проект перенесён в `tools/WinxRemix` 13 сентября 2026 по поручению пользователя.
Здесь хранятся наш адаптер, скрипты, тесты и патчи. Общая восстановленная логика
по-прежнему подключается из `Sparkplug/` и `Winx/`.

## Зависимости и сборка

Исходники Remix получаем из [официального репозитория NVIDIA](https://github.com/NVIDIAGameWorks/dxvk-remix).
Версия и контрольные суммы закреплены в [dependencies.json](dependencies.json).
Reference checkout находится в `local-data/rtx-remix/upstream/dxvk-remix`;
наши патчи применяются к отдельным рабочим копиям в `local-data/rtx-remix`.
Полного дерева Remix или копии его функций в исходниках этого проекта нет.

Из корня репозитория:

```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File tools/WinxRemix/Prepare-Dependencies.ps1
powershell -NoProfile -ExecutionPolicy Bypass -File tools/WinxRemix/Build-Probe.ps1 -OutputDirectory local-data/rtx-remix/build-check
powershell -NoProfile -ExecutionPolicy Bypass -File tools/WinxRemix/Run-WinxRemix.ps1 -Mode RTX
```

Первый шаг получает недостающие внешние исходники. Следующие сборки проверяют
зависимости без сети. Нужны Python 3, Git, MSVC C++ и Windows SDK. Команда сборки
выше создаёт проверочную DLL; запуск использует уже установленный прототип.
Готовый runtime и файлы игры подготавливаются отдельно.

xxHash скачивается у автора с проверкой SHA-256. Фрагмент преобразования света
генерируется из закреплённого оригинала Remix с нашей явно записанной адаптацией.
Оба заголовка находятся в `local-data/rtx-remix/dependencies/include/third-party`,
вне Git. Тестовые снимки сохраняют использованные заголовки и сведения об их
происхождении рядом с результатами в `local-data`.

Исторические отчёты и JSON-manifests сохраняют прежние пути `research/rtx-remix`
и исходные хеши. Они описывают соответствующие опыты, не текущее расположение.
Актуальная [готовность](../../docs/research/winx-remix-progress.md) учитывается отдельно.

Проверка переноса: x86 DLL собрана из нового пути; NativeSkinVertices — 122
CPU-проверки, NativeLightSource — 50, SceneLights — 110. Проверены 39 PowerShell
скриптов и отдельные CLI анализаторов (233/95/42/16/15 проверок). Общий
`unittest discover` не является их штатным запуском: два теста требуют явного
свежего `--fixture-dir`; повтор через их CLI прошёл. C++-файлы и патчи перенесены
без изменения байтов. Игра для проверки каталогов не запускалась.
...
Оригинальные исходники Remix служат внешним эталоном; наши изменения bridge и
renderer хранятся отдельными патчами с проверками. Перенос каталогов не меняет EXE игры.
...
`powershell -NoProfile -ExecutionPolicy Bypass -File tools/WinxRemix/Test-SurfaceMaterial.ps1`
...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File tools/WinxRemix/Build-Probe.ps1
powershell -NoProfile -ExecutionPolicy Bypass -File tools/WinxRemix/Start-Probe.ps1 -Name fresh-test -ExplicitMipLevels -ResubmitTextures -Raytracing -OrthographicUi -FitWindow -ViewportScale -MenuBackground -SkyLayers
```
...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File tools/WinxRemix/Run-WinxRemix.ps1 -Mode RTX -DebugMenu -ShaderAudit
python -B tools/WinxRemix/analyze_shader_audit.py local-data/rtx-remix/runs/ИМЯ-ЗАПУСКА
```
...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File tools/WinxRemix/Restore-StockRemix.ps1
```
...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File tools/WinxRemix/Run-WinxRemix.ps1 -Mode RTX -DebugMenu -SceneLights -MaterialAudit -StartLevel 4 -Windowed
python tools/WinxRemix/analyze_material_audit.py local-data/rtx-remix/runs/ИМЯ-ЗАПУСКА --verify-shaders
powershell -NoProfile -ExecutionPolicy Bypass -File tools/WinxRemix/Test-MaterialAudit.ps1
```
...
`python tools/WinxRemix/analyze_remix_material.py --native ПАПКА --remix ПАПКА --output НОВЫЙ-JSON`.
