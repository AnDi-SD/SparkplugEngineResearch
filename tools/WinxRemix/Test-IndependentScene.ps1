...
$build=Join-Path $root "local-data/rtx-remix/independent-scene-tests/$Name"
...
$source=Join-Path $snapshot 'test_independent_scene.cpp'
# Ordinary CPU fixture executable on owned records; no original game functions.
$commands=@"
@echo off
call "$environmentScript" x86
if errorlevel 1 exit /b %errorlevel%
cl /I"$dependencyInclude" /nologo /std:c++17 /EHsc /MT /O2 /W4 /I"$include" "$source" /Fe:test_independent_scene.exe /link user32.lib
exit /b %errorlevel%
"@
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw "Independent scene build failed; see $build/build.log" }
    if ($SkipRun) { Write-Output "Built CPU independent scene fixture: $build"; return }
    $exe=Join-Path $build 'test_independent_scene.exe'
    ...
    $process.Dispose()
    if ($exitCode -ne 0) { throw "Independent scene fixture failed ($exitCode); see $build/stderr.log" }
    $result=Get-Content -LiteralPath (Join-Path $build 'result.json') -Raw | ConvertFrom-Json
    if ($result.status -ne 'PASS') { throw 'Independent scene fixture did not report PASS' }
