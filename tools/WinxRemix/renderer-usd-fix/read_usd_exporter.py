...
try:
    ...
    for nonuniform in (False,True):
      for reduced in (False,True):
        ...
        check(name+' stage metadata',bool(stage) and stage.GetStartTimeCode()==0 and stage.GetEndTimeCode()==2 and stage.GetTimeCodesPerSecond()==24
        ...
        check(name+' camera single op with three samples',len(ops)==1 and ops[0].GetAttr().GetTimeSamples()==[0.,1.,2.])
        for t in range(3):near(name+' camera '+str(t),value(ops[0].Get(t)),value(Gf.Matrix4d(1)))
        ...
        for prim in stage.Traverse():
            ...
            expected_weights=[x for i in slots for x in (weights[c][i*n:(i+1)*n] if i is not None else [0.]*n)]
            expected_joints=[(i+k)%4 if i is not None else 0 for i in slots for k in range(n)]
            if options.default_joints:expected_joints=[k for i in slots for k in range(n)]
            ...
            check(label+' indices and influence width',value(js.Get())==expected_joints and ws.GetElementSize()==js.GetElementSize()==n and ws.GetInterpolation()==js.GetInterpolation()=='vertex')
            ...
            expected_names=['root','root/joint1','root/joint2','root/joint3'] if options.topology=='star' else ['joint0','joint1','joint2','joint3']
            ...
            for t in range(3):check(label+' visibility '+str(t),(UsdGeom.Imageable(prim).ComputeVisibility(t)=='inherited')==(not prototype and (not late or t==1)))
            if prototype:continue
            parent=prim.GetParent();op=UsdGeom.Xformable(parent).GetOrderedXformOps()
            check(label+' single instance transform op',len(op)==1 and op[0].GetAttr().GetTimeSamples()==([1.] if late else [0.,1.,2.]))
            ...
            for attr in (anim_schema.GetTranslationsAttr(),anim_schema.GetRotationsAttr(),anim_schema.GetScalesAttr()):
                check(label+' '+attr.GetName()+' sample times',attr.GetTimeSamples()==([1.] if late else [0.,1.,2.]))
            for t in ([1] if late else range(3)):
                prefix=label+' t'+str(t)
                local=Gf.Matrix4d(1);local.SetTranslate(Gf.Vec3d(.125*t,.25 if late else 0,0))
                near(prefix+' instance matrix',value(op[0].Get(t)),value(local))
                expected_palette=[pose(t,j,nonuniform) for j in range(4)]
                ...
                for i in slots:
                    ...
                    transformed=[transform(logical[i],expected_palette[k if options.default_joints else (i+k)%4]) for k in range(n)]
...
report['scope']='Own CPU fixture: actual USD export/readback, 1..4 influences, signed explicit weights, 4-joint palette, animated scales/rotations, reduction, visibility, 33 constants and 12 own DDS references. No GPU/capturer/texture writer/game qualification.'
report['jointIndices']='default D3D9 0..N-1' if options.default_joints else 'explicit per vertex'
