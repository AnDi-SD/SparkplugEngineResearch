param([Parameter(Mandatory=$true)][ValidatePattern('^[a-zA-Z0-9_-]+$')][string]$Name,
      [ValidateSet('x86','x64')][string]$Platform='x86', [switch]$SkipRun)
$ErrorActionPreference='Stop'
$root=[IO.Path]::GetFullPath((Join-Path $PSScriptRoot '../..'))
$dependencyInclude = & (Join-Path $PSScriptRoot 'Prepare-Dependencies.ps1') -Offline
$build=Join-Path $root "local-data/rtx-remix/skin-packet-submit-tests/$Name"
...
$source=Join-Path $snapshot 'test_skin_packet_submit.cpp'
# Ordinary CPU fixture executable. Owned packets exercise the shared API resource
# cache and recording Create/Draw/Destroy callbacks; no game or COM method runs.
$commands=@"
@echo off
call "$environmentScript" $Platform
if errorlevel 1 exit /b %errorlevel%
cl /I"$dependencyInclude" /nologo /std:c++17 /EHsc /MT /O2 /W4 /I"$include" "$source" /Fe:test_skin_packet_submit.exe /link user32.lib
exit /b %errorlevel%
"@
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw "Skin packet submit build failed; see $build/build.log" }
    if ($SkipRun) { Write-Output "Built CPU skin packet submit fixture: $build"; return }
    $exe=Join-Path $build 'test_skin_packet_submit.exe'
    ...
    $process.Dispose()
    if ($exitCode -ne 0) { throw "Skin packet submit fixture failed ($exitCode); see $build/stderr.log" }
    $result=Get-Content -LiteralPath (Join-Path $build 'result.json') -Raw | ConvertFrom-Json
    if ($result.status -ne 'PASS') { throw 'Skin packet submit fixture did not report PASS' }
    @{ schema=1; platform=$Platform; gpu=$false; nativeGameCodeExecuted=$false; watchdogSeconds=30; exeSha256=(Get-FileHash -LiteralPath $exe).Hash; result=$result } |
