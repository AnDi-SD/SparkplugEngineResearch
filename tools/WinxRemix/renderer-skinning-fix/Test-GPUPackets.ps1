param([Parameter(Mandatory=$true)][ValidatePattern('^[a-zA-Z0-9_-]+$')][string]$Name,
      [Parameter(Mandatory=$true)][ValidatePattern('^[a-zA-Z0-9_-]+(,[a-zA-Z0-9_-]+)*$')][string]$Runs)
$ErrorActionPreference='Stop'
$root=[IO.Path]::GetFullPath((Join-Path $PSScriptRoot '../../..'))
$build=Join-Path $root "local-data/rtx-remix/renderer-skinning-fix-tests/$Name"
& python (Join-Path $PSScriptRoot 'prepare_packet_gpu.py') $build --runs $Runs
if ($LASTEXITCODE -ne 0) { throw 'GPU source snapshot preparation failed' }
$manifest=Get-Content -LiteralPath (Join-Path $build 'packet-gpu-source.json') -Raw | ConvertFrom-Json
...
cl /nologo /std:c++17 /EHsc /MT /O2 /arch:AVX2 /DNOMINMAX /DWIN32_LEAN_AND_MEAN /I"$build/source/vulkan-headers" /I"$build/source" /I"$build/source/include" /I"$build/source/src/dxvk/shaders" "$build/test_gpu_skin_packets.cpp" /Fe:test_gpu_skin_packets.exe /link "$build/vulkan-1.lib" user32.lib
exit /b %errorlevel%
"@
...
try {
    ...
    $process.StartInfo.FileName=Join-Path $build 'test_gpu_skin_packets.exe'
    $process.StartInfo.Arguments='"'+(Join-Path $build 'gpu_skinning.spv')+'" "'+(Join-Path $build 'packets')+'" '+$manifest.packetCount
    ...
    $result=$events[-1]
    if ($result.status -ne 'PASS' -or $result.dispatches -ne $manifest.expectedDispatches -or $result.capturedPackets -ne $manifest.packetCount -or $result.capturedVertices -ne $manifest.capturedVertices -or $result.syntheticCases -ne $manifest.syntheticCases -or -not $result.resourcesDestroyed) { throw 'GPU fixture did not finish every dispatch and cleanup' }
    $result | ConvertTo-Json -Depth 5 | Set-Content -LiteralPath (Join-Path $build 'result.json') -Encoding UTF8
