...
$build=Join-Path $root "local-data/rtx-remix/native-skin-source-tests/$Name"
...
$source=Join-Path $snapshot 'test_native_skin_source.cpp'
...
cl /I"$dependencyInclude" /nologo /std:c++17 /EHsc /MT /O2 /W4 /I"$include" "$source" /Fe:test_native_skin_source.exe /link user32.lib
exit /b %errorlevel%
"@
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw "Native skin source build failed; see $build/build.log" }
    if ($SkipRun) { Write-Output "Built CPU native skin source fixture: $build"; return }
    $exe=Join-Path $build 'test_native_skin_source.exe'
    ...
    $process.Dispose()
    if ($exitCode -ne 0) { throw "Native skin source fixture failed ($exitCode); see $build/stderr.log" }
    $result=Get-Content -LiteralPath (Join-Path $build 'result.json') -Raw | ConvertFrom-Json
    if ($result.status -ne 'PASS') { throw 'Native skin source fixture did not report PASS' }
