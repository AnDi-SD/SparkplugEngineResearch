...
foreach($relative in @('winx_scene_lights.h','test_light_ownership.cpp')) {
  Copy-Item -LiteralPath (Join-Path $PSScriptRoot $relative) -Destination (Join-Path $source $relative)
}
Get-ChildItem -LiteralPath (Join-Path $dependencyInclude 'third-party') -File | ForEach-Object { Copy-Item -LiteralPath $_.FullName -Destination (Join-Path $source 'third-party') }
...
cl /I"$dependencyInclude" /nologo /std:c++17 /EHsc /MT /O2 /W4 /I"$apiInclude" "$source\test_light_ownership.cpp" /Fe:test_light_ownership.exe
exit /b %errorlevel%
"@
