...
public static class SmoMeshResourceReplacer
{
        ...
        Matrix4x4? referenceWorldTransform = null)
    {
        ...
        WriteUInt32(
            result,
            payload + 8,
            checked((uint)(vertexCount * template.RuntimeStride)));
        ...
        for (int triangle = 0; triangle < indexCount; triangle += 3)
        {
            WriteUInt16(
                result,
                indices + triangle * 2,
                checked((ushort)prepared.TriangleIndices[triangle]));
            WriteUInt16(
                result,
                indices + (triangle + 1) * 2,
                checked((ushort)prepared.TriangleIndices[triangle + 1]));
            WriteUInt16(
                result,
                indices + (triangle + 2) * 2,
                checked((ushort)prepared.TriangleIndices[triangle + 2]));
        }
        ...
        for (int vertex = 0; vertex < vertexCount; vertex++)
        {
                ...
                prepared.Normals.Length == vertexCount)
            {
                WriteVector3(
                    result,
                    offset + normalOffset,
                    prepared.Normals[vertex]);
            }
            if (layout.TextureCoordinate0Offset is int uvOffset &&
                prepared.TextureCoordinates.Length == vertexCount)
            {
                WriteSingle(
                    result,
                    offset + uvOffset,
                    prepared.TextureCoordinates[vertex].X);
                WriteSingle(
                    result,
                    offset + uvOffset + 4,
            ...
            if (layout.DiffuseArgbOffset is int colorOffset)
            {
                WriteUInt32(
                    result,
                    offset + colorOffset,
                    prepared.DiffuseColorsArgb[vertex]);
            }
            if (layout.BlendWeightsOffset is int weightsOffset &&
                layout.BlendIndicesOffset is int bonesOffset)
            {
                Vector4 weights = prepared.BlendWeights[vertex];
                WriteSingle(result, offset + weightsOffset, weights.X);
                WriteSingle(result, offset + weightsOffset + 4, weights.Y);
                WriteSingle(result, offset + weightsOffset + 8, weights.Z);
                WriteSingle(result, offset + weightsOffset + 12, weights.W);
                SmoBlendIndices bones = prepared.BlendIndices[vertex];
                result[offset + bonesOffset] = bones.X;
                result[offset + bonesOffset + 1] = bones.Y;
                result[offset + bonesOffset + 2] = bones.Z;
