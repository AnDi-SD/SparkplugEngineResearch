...
internal static class AlphaBranchRegression
{
        ...
        string context)
    {
        ...
        RunSingleTextureAutomaticAlpha(
            target,
            preparedScene,
            outputPath,
            context + " single-texture Auto RGBA",
            fixture);
    ...
    private static void RunSingleTextureAutomaticAlpha(
        SmoDocument target,
        ImportedScene preparedScene,
        string outputPath,
        string context,
    ...
    {
        AlphaRenderableFixture expected = fixture.AlphaRenderables.Single(item =>
            item.TextureName.Equals("mat5.png", StringComparison.OrdinalIgnoreCase));
        ImportedMesh sourceMesh = preparedScene.Meshes[expected.SourceMeshKey];
        ImportedMaterial sourceMaterial = preparedScene.Materials[sourceMesh.MaterialIndex];
        ImportedTexture sourceTexture =
            preparedScene.Textures[sourceMaterial.BaseColorTextureIndex];
        if (!TextureContainsTransparency(sourceTexture))
        {
            throw new InvalidOperationException(
                $"{context}: source fixture unexpectedly has no texture alpha.");
        }

        var singleScene = new ImportedScene(
            [sourceMesh with { MaterialIndex = 0 }],
            [sourceTexture],
            [sourceMaterial with
            {
                BaseColorTextureName = sourceTexture.Name,
        ...
        GlbSkinTransferPlan plan = SmoSkinnedGlbReplacer.Analyze(
            ...
            singleScene,
            SkinnedTextureTransferMode.ImportDonor);
        if (!plan.CanReplace || !plan.Messages.Any(message => message.Contains(
                "Detected real transparency", StringComparison.Ordinal)))
        {
            throw new InvalidOperationException(
                $"{context}: ordinary RGBA texture was not detected automatically: " +
                string.Join(" | ", plan.Messages));
        }
        ...
        try
        {
            ...
            {
                throw new InvalidOperationException(
                    $"{context}: Auto mode produced no native alpha mesh branch.");
            }
            TriangleMultiset actual = BuildOutputTriangles(output, alphaMeshes);
            expected.Triangles.AssertEqual(actual, context + " alpha geometry");
            IReadOnlyDictionary<int, SmoTextureBinding> bindings =
                SmoTextureBindingResolver.ResolveAll(output);
            foreach (SmoObjectEntry mesh in alphaMeshes)
            {
                if (!bindings.TryGetValue(mesh.Index, out SmoTextureBinding? binding) ||
                    ...
                    !binding.Texture.Bgra32Pixels.Span
                        .ToArray()
                        .Where((_, index) => index % 4 == 3)
                        .Any(alpha => alpha < byte.MaxValue))
                {
                    throw new InvalidOperationException(
                        $"{context}: saved alpha branch has no RGBA texture or " +
                        "native transparent material state.");
                }
            }
            Console.WriteLine(
                $"  {context} PASS: triangles={actual.Count}; RGBA and native " +
                "alpha material branch verified.");
        }
        finally
        {
            if (File.Exists(outputPath))
