...
Статус: `SmoImporter 0.4.1`. Реализованы append-only путь **SMO → SMO**,
...
FBX проходит этот же путь через прямой Autodesk FBX SDK reader без временного GLB.
Native bridge нормализует систему координат и единицы, строит joint palette из
skin clusters и передаёт inverse/world/local bind matrices. В обычном режиме при
наличии skin несвязанные helper meshes намеренно исключаются. После этого применяются
все те же проверки joints, bind pose, palettes и безопасной RGB-текстуры.
...
uniform scale, конечным XYZ rotation и конечным XYZ translation. Явное значение полностью
заменяет automatic alignment, а не компонуется с ним. GUI использует height/center fit
по полным bounds только как старт и позволяет исправить scale/rotation/translation
вручную числовыми полями или screen-space gizmo;
крылья, удалённые аксессуары и другие выбросы могут сместить эту начальную оценку.
Core fail-loud отклоняет non-finite rotation, non-positive/non-finite scale,
non-finite translation и необратимую матрицу.
...
4. использует явный абсолютный scale + rotation + translation либо, если вызывающий код его не
   передал, вычисляет conservative automatic alignment по robust bounds выбранной
   поверхности. Поворот и отражение автоматически не угадываются: после явного
   rotation donor обязан быть upright, Y-up, unmirrored и same-facing с target;
...
### Защищённые semantic vertex regions

Анатомический эллипсоид выше только меняет относительный capsule score. Для головы и
кистей этого недостаточно: даже небольшая примесь длинной кости может сильно увести
крайнюю вершину в анимации. Поэтому после обычной генерации weights отдельный
`GeneratedSkinningSemanticRegionResolver` может точно заменить influences внутри
трёх semantic regions выбранной smooth-body surface: `Head`, `LeftHand` и `RightHand`.

Калибровка строится из четырёх независимых источников:

- точного `TargetRigDefinition` и ближайшего deform-родителя semantic anchor;
- декодированных исходных target weights: строгих samples поддерева anchor и
  samples перехода anchor/parent;
- bone-local axial/lateral/forward frame, перенесённого в текущую fitting pose;
- точной donor body topology, включая adjacency, сторону тела и совпадающие позиции
  attribute-seam vertices.

Имена donor meshes, materials, nodes, файлов и моделей не входят ни в классификацию,
ни в fallback. Единственные имена в semantic-контракте относятся к deform-семантике
самого target rig: `Head`, `L_Hand`, `R_Hand`; для головы ближайшим deform-родителем
обязан быть `Neck`, для кисти используется фактический ближайший deform-родитель.

Для `Head` исходные target samples определяют неизменяемую bone-local плоскость
среза и консервативные пределы размера. Все high-confidence seed выше плоскости
группируются по donor adjacency внутри ровно одного выбранного владельца body
surface. Primary-кластер выбирается по доказанной границе шеи. Остальные
seed-кластеры того же владельца принимаются только целиком: они обязаны быть
компактными, полностью лежать в ограниченном head envelope и для каждой позиции
быть ближе к primary Head surface, чем к оставшейся body surface. Primary и все
принятые secondary-кластеры образуют единый доказанный head lobe. Каждая его
вершина получает exact one-hot `Head`; внутреннего Head/Neck transition в голове
нет. Необъяснённый seed, внутренняя граница или достижение distal safety bound
отменяют Head-region целиком. Эллипсоид `p = 2` проверяет полный объединённый
membership; ручная правка повторно проверяет каждую вершину, поэтому частично
морфируемого лица быть не может.

Head/Neck transition строится отдельно и не пересекается с head lobe. Для каждой
точной позиции его граничного кольца должна существовать внешняя same-owner соседняя
вершина ниже неизменяемого среза; отсутствие хотя бы одной такой вершины означает
неполное покрытие и отменяет Head-region целиком. Эти внешние seed образуют шейный
воротник. Его расстояния вычисляются ограниченным Dijkstra-обходом по поверхности
шеи с радиальными и axial safety bounds. Ширина адаптируется к одному локальному
низкополигональному шагу и ограничивается target-derived осевым радиусом `Head`.
Совпадающие attribute-seam позиции обязаны иметь одинаковые расстояния и веса;
незамкнутый seam отклоняет область. Только в этом внешнем воротнике допускается
смешанное влияние `Head ↔ Neck`, тогда как весь head lobe остаётся exact one-hot.

Для `LeftHand`/`RightHand` target-derived объём сначала центрируется в donor-local
геометрии. Предпочтительный evidence строится из уже вычисленного capsule path:
берутся только веса фактического target Hand-subtree. Если они не образуют
недегенеративный кластер, fallback строит конечный target-axis topology corridor,
разбивает его по adjacency и объединяет все co-nearest к оси кластеры. Ни один путь
не использует donor mesh/material/name. Evidence задаёт только поперечный центр и
минимальный осевой сдвиг, ограниченный target transition; окончательный membership
из него не копируется.

После recenter все evidence-вершины, которые действительно оказались строгими
дистальными seed внутри superellipsoid `p = 4`, обязаны принадлежать одному owner и
одной связной лопасти. Полностью проксимальные пересечения corridor считаются
запястьем/предплечьем, а не Hand evidence. Захват распространяется по adjacency
выбранного smooth body на нужной стороне до конечной safety-плоскости. При встрече
этой плоскости область отклоняется вместо обрезания. Так получается полный distal
topology-lobe от доказанной границы запястья до всех кончиков. Округлая норма `p = 4`
служит containment guard, поэтому широкие дистальные углы не выпадают из-за
эллиптического cap. Геодезическое расстояние от wrist boundary задаёт только
переход `nearest proximal deform ↔ Hand`.

В core кисти фактические прямые deform-ветви target под `Hand` используются как
приблизительные дорожки пальцев. Из каждой ветви берутся первые две или три кости;
donor-пальцы по именам, порядку и отдельной topology не распознаются. Core-вершина
попадает в ближайшую поперечную дорожку, а дистальный кончик каждой дорожки отдельно
задаёт её наблюдаемую длину. После стабильной зоны ладони нормированный прогресс
последовательно смешивает `Hand`, первую, вторую и третью кости; одновременно активны
не более двух соседних костей одной иерархической цепи. Так кончик может полностью
следовать последней фаланге и сгибаться существующими target-анимациями, даже если
приблизительное разделение donor-пальцев неточно. Если target не доказывает безопасные
ветви, полный захваченный core остаётся rigid `Hand`. Для головы обязательны доказанный one-hot
head lobe и отдельный полностью покрывающий его границу внешний воротник; для кистей
обязательны core и доказанный transition. Ни одна посторонняя кость не может войти
в semantic weights; за пределами точного captured membership прежние generated
weights не меняются.

После успешного Head-захвата отдельно проверяются detached-компоненты, не входящие в
выбранный smooth body. Компонент становится semantic Head companion только целиком:
он должен быть компактным, укладываться в ограниченно расширенный head envelope и
быть достаточно близким к поверхности Head core. Совпадение с transition/outside
вершиной запрещает назначение. Строгий кандидат обязан для каждой позиции иметь
Head-core ближайшей body-поверхностью. Соседняя часть составного глаза или рта может
присоединиться только одним нерекурсивным шагом к уже строго доказанному компоненту,
причём не менее четверти её позиций должны самостоятельно предпочитать Head-core.
Поэтому глаза, зубы и небольшая лицевая деталь следуют голове без морфинга, а
крылья, длинные волосы, детали касающейся лица руки и удалённые аксессуары остаются
на обычном rigid attachment path. Никакие имена donor mesh/material/file в этой
проверке не используются; подтверждённое ручное назначение имеет приоритет.

Каждая область может быть отключена или скорректирована относительно автоматического
объёма через `AxialOffset`, `AxialScale` и `RadialScale`. Допустимые пределы выводятся
из самой калибровки и повторно проверяются Core. Overrides содержат fingerprints
target rig, donor geometry/topology, alignment и fitting pose: перенос stale membership
между ревизиями блокируется до изменения writer input.

Недостаточные target samples, неизвестная/неоднозначная semantic bone, отсутствие
связного donor core, отсутствие полного внешнего same-owner воротника головы,
overlap областей и расхождение совпадающих seam-вершин дают `UnsafeCalibration`.
Это консервативный no-op для всей сомнительной области, а не частичное применение
или догадка.
Неполная граница кисти также даёт консервативный no-op. Поскольку стадия
переписывает только generated joint
indices/weights, она не меняет positions, normals, triangles, UV, material mapping
или texture payload.

Adaptive palette compatibility выполняет от одного до трёх legacy probes и один
финальный проход, но semantic resolution разрешён только в финальном: runtime
инвариант требует ровно один такой вызов на публичный `Prepare`. Точные nearest
distance для secondary Head shells и detached companions вычисляет
детерминированный balanced k-d tree; прежние eager all-pairs scans не используются.
Head half-space разбивается на topology-кластеры после одной детерминированной
сортировки за `O(V log V + E)`: повторной сортировки уменьшающегося набора для
каждого островка нет, поэтому фрагментированная comb/checkerboard-геометрия не
создаёт квадратичного зависания.

GUI не добавляет третий режим редактора: секция **Защищённые области** находится
внутри существующего режима **Человек**. Голова/кисти выбираются отдельно, для рук
есть зеркальная правка; **Положение**, **Длина** и **Толщина** управляют тремя
adjustment-параметрами. Голубой wireframe показывает объём, синие точки — ядро,
жёлтые — захваченный переход (для `Head` — внешний воротник на шее), фиолетовые —
отделённые semantic-компаньоны. Для кисти панель сообщает число доступных пальцевых
ветвей, число подвижных core-вершин и максимальную долю артикуляции. Ползунки
создают debounced live draft, но commit
транзакционен: фоновый preview заранее выполняет preparation и точный writer `Analyze`,
а **Применить** фиксирует тот же проверенный immutable result без повторного расчёта в UI-потоке.
Только после успеха обеих проверок заменяется writer input. **Сброс** возвращает последнее
применённое состояние, **Авто** — нулевой offset и единичные scales. Commit не закрывает
редактор: следующий draft строится поверх только что применённого результата. Пока draft
не применён, прежние PreparedScene, подтверждение и plan сохраняются и снова становятся
доступны после **Сброс**.
Live draft считается вне WPF Dispatcher; одновременно допускается одна фоновая
задача, а revision check запрещает устаревшему результату заменить текущий draft.

Алгоритм не распознаёт конкретные donor-пальцы и не содержит исключений для моделей.
Захваченная кисть плавно соединяется у запястья и получает приблизительные weights
на target-derived ветви пальцев с двумя-тремя фалангами в каждой. Это отдельный
механизм от **Жёстких деталей** ниже: semantic region может работать внутри
сваренной smooth surface, не превращая её в detached rigid component.
...
payload, поэтому GUI сохраняет его при изменении позы, uniform scale/rotation/translation,
...
только применённые uniform scale/rotation/translation. Его `SelectBody` выполняет только
...
Редактирование scale/rotation/translation создаёт pending
...
как donor alignment остаётся в canonical target space. Scale/rotation/translation уже входят
