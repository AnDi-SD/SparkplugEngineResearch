...
## 2.2.0 — кандидат, 2026-09-08

- В GUI включены проверенные RGBA replacement, resize и сохранение копии SMO
  для embedded PC BGRA single-mip. Неподдерживаемые representations read-only.
- Структурный FFPS writer заменяет marker/fixed-offset эвристики версии2.1;
  обновляет каталог, размеры и offsets, поддерживает точные NPOT-размеры.
- PNG variants, точные output pixels и приём поддержанных записей PC-кодом
  проверены. Подробности — в RELEASE_NOTES_2.2.0.md.
