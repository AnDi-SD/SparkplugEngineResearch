...
public sealed partial class MainWindow : Window, INotifyPropertyChanged
{
    ...
    public string Summary => _document is null
        ...
        : $"{_document.Textures.Count} текстур · {_document.Length:N0} байт · " +
          ...
          $"{_document.UnsupportedTextures.Count} неподдерживаемых ресурсов";
    ...
    public bool SafeMode
    {
        ...
        set
        {
            ...
            Status = value
                ...
                : "Расширенный предел: сторона до 16384 пикселей, до 128 МиБ несжатых пикселей на изображение.";
        }
    }
    ...
    private async void OpenFile_Click(object? sender, RoutedEventArgs e)
    {
        ...
        try
        {
            LoadDocument(files[0].Path.LocalPath);
        }
        catch (Exception ex)
        {
    ...
    internal void LoadDocument(string path)
    {
        ...
        Status = document.Textures.Count == 0
            ...
            : $"Файл разобран и проверен. Замена: полный RGBA; режимы смешивания материалов сохраняются.";
        NotifyDocumentState();
    }
    ...
    {
        if (_busy || _document is null || _sourcePath is null || !HasReplacements)
            return;
        ...
        try
        {
            await SaveCopyAsync(file.Path.LocalPath);
        }
        catch (Exception ex)
        {
    ...
    internal async Task<SmoViewer.Core.SmoVerifiedOutputInstallResult> SaveCopyAsync(string outputPath)
    {
        if (_busy || _document is null || _sourcePath is null || !HasReplacements)
            throw new InvalidOperationException("Сначала откройте SMO и выберите замену.");
        SmoDocument document = _document;
        string sourcePath = _sourcePath;
        var selected = Rows.Where(row => row.ReplacementPath is not null).ToArray();
        ...
        var replacements = selected.ToDictionary(row => row.Texture.Index, row => row.ReplacementPath!);
        string[] protectedPaths = new[] { sourcePath }.Concat(replacements.Values).ToArray();
        _busy = true;
        OnPropertyChanged(nameof(IsIdle));
        Status = "Пересборка и проверка копии…";
        try
        {
            var installed = await Task.Run(() =>
            {
                byte[] result = document.Repack(replacements);
                return SmoViewer.Core.SmoVerifiedOutputInstaller.Install(outputPath, result,
                    temporary => { _ = SmoDocument.Load(temporary); }, protectedPaths);
            });
            Status = "Новый SMO сохранён и повторно проверен." +
                (installed.BackupPath is null ? "" : $" Предыдущая копия: {installed.BackupPath}");
            return installed;
        }
        finally
        {
    ...
    private async void ChooseReplacement_Click(object? sender, RoutedEventArgs e)
    {
        ...
        try
        {
            SelectReplacement(row, files[0].Path.LocalPath);
        }
        catch (Exception ex)
        {
    ...
    internal void SelectReplacement(TextureRowViewModel row, string path)
    {
        if (_busy || !Rows.Contains(row))
            throw new InvalidOperationException("Текстура не принадлежит открытому документу.");
        using SixLabors.ImageSharp.Image<Rgba32> image =
            TextureImageLoader.Load(path);
        ValidateDimensions(row.Texture, image.Width, image.Height);
        row.SetReplacement(path, ToPreviewBitmap(image));
    ...
    {
        TextureImageLoader.ValidateDimensions(width, height);
        if (!texture.CanReplace)
            throw new NotSupportedException(texture.ReplacementIssue);
            ...
            foreach (TextureRowViewModel row in Rows)
            {
                ...
                if (row.ReplacementPath is not null)
                {
                    using SixLabors.ImageSharp.Image<Rgba32> replacement =
                        TextureImageLoader.Load(row.ReplacementPath);
                    if (SelectedPreviewMode == "Окраска модели")
