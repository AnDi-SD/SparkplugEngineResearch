...
public sealed class TextureRowViewModel : INotifyPropertyChanged, IDisposable
{
    ...
    public string Header =>
        $"{Texture.FileName} · {Texture.Width}×{Texture.Height} · {Texture.RepresentationName}";
    ...
    public string MaterialDetails => Texture.Material is { } material
        ...
        : Texture.MaterialIssue ?? "Владелец материала не найден.";
    public string ResizeNotice =>
        Texture.ReplacementIssue ??
        "Замена полного RGBA с сохранением размера изображения. Режим смешивания материала сохраняется. " +
        "Обычный предел — 4096²; расширенный — сторона до 16384 и до 128 МиБ пикселей.";
