...
if (args is ["--material-inspection-regression", string materialBook, string materialCurrent, string materialOutput])
{
    try { return MaterialInspectionRegression.Run(materialBook, materialCurrent, materialOutput); }
    catch (Exception error) { Console.Error.WriteLine(error); return 1; }
}

if (args is ["--material-binding-guards", string bindingSource, string bindingOutput])
{
    try { return MaterialInspectionRegression.RunBindingGuards(bindingSource, bindingOutput); }
    catch (Exception error) { Console.Error.WriteLine(error); return 1; }
}

if (args is ["--field-header-regression", string fieldHeaderOutput])
    return FieldHeaderRegression.Run(fieldHeaderOutput);

if (args.FirstOrDefault() == "--focused")
{
    try
    {
        FocusedTextureRegression.Run(args.Skip(1).ToArray());
        return 0;
    }
...
try
{
    ...
    bool descriptorValidationChecked = false;
    ...
    foreach (string file in Directory.EnumerateFiles(samples, "*.smo").Order())
    {
        ...
        foreach (TextureInfo texture in document.Textures)
        {
            ...
            {
                int mipHeightHighByteOffset = checked(texture.BlockOffset + 0x3C);
                ...
                Assert(original[mipHeightHighByteOffset] == 0,
                    $"{name}, текстура {texture.Index}: старший байт mipHeight на +0x3C не равен 00.");
                ...
                if (!descriptorValidationChecked)
                {
                    byte[] invalidHeight = (byte[])original.Clone();
                    invalidHeight[mipHeightHighByteOffset] = 0xFF;
                    SmoDocument invalidDocument = SmoDocument.Parse(invalidHeight);
                    Assert(invalidDocument.UnsupportedTextures.Count == 1 &&
                        !invalidDocument.Textures.Any(item => item.BlockOffset == texture.BlockOffset),
                        "parser не отметил повреждённую высоту mip как неподдерживаемый ресурс.");
                    descriptorValidationChecked = true;
                }
            }
            ...
            if (texture.FormatCode is 0x32E3 or 0x43E3)
                Assert(repacked[texture.BlockOffset + 0x3C] == 0,
                    $"{name}, текстура {texture.Index}: round-trip изменил старший байт mipHeight на +0x3C.");
            checkedTextures++;
        }
    ...
    Assert(descriptorValidationChecked,
        "В Samples не найден формат 0x32E3/0x43E3 для проверки старшего байта mipHeight.");
    ...
    Assert(rectangularFile[rectangularBlock + 0x3C] == 0,
        "Прямоугольный resize изменил старшего байта mipHeight на +0x3C.");
    AssertRawBgraPixel(
    ...
    Assert(resizedFile[resizedTexture.BlockOffset + 0x3C] ==
        ...
        "Resize изменил старшего байта mipHeight на +0x3C.");
    AssertRawBgraPixel(
