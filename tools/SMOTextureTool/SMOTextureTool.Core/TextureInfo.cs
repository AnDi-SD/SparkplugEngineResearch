    ...
    TextureChannelInfo Channels)
{
    ...
    // Codec projection for display/export; PixelDataOffset still locates the
    // untouched stored bytes. Empty means direct BGRA storage.
    internal ReadOnlyMemory<byte> BgraPreview { get; init; }
    public string RepresentationName { get; init; } = "BGRA32";
    public int MipLevelCount { get; init; } = 1;
    public string? ReplacementIssue { get; init; }
    public bool CanReplace => ObjectIndex >= 0 && ReplacementIssue is null;
    public bool CanResize => CanReplace;
    public MaterialReferenceInfo? Material { get; init; }
    /// <summary>Material-view diagnostics are independent of texture replacement support.</summary>
    public string? MaterialIssue { get; init; }
