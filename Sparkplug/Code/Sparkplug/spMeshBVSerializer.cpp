...
bool spMeshBVSerializer::ReadPayloadForAnalysis(spSerializerReadContextForAnalysis& context,
    ...
    while(const auto* header=cursor.Next()) {
        ...
        if(header->fieldID==0) {
            ...
            if(!data->indices_->ReadForAnalysis(stream,header->payloadSize))return cursor.Fail("Invalid MeshBV index buffer");
            std::uint32_t position=0;
            if(!stream.GetCurrentPosition(position)||position<header->dataStreamPosition
                ||position-header->dataStreamPosition>header->payloadSize)return cursor.Fail("MeshBV index extent overflow");
            if(!data->vertices_->ReadForAnalysis(stream,header->payloadSize-(position-header->dataStreamPosition)))
                return cursor.Fail("Invalid MeshBV vertex buffer");
            current=data.get();
