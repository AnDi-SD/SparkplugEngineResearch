# `spPS2AsyncFileStreamManager`: крупный PS2 leaf с dormant queue

Статус: глубокий статический срез выполнен 4 сентября 2026 года. Доказаны
identity, factory/clone/destruction, размер `0x37A0`, layout пятидесяти request
records, полный public request и update consumer. Существенная неизвестность
сохранена явно: executable не содержит producer записи `queuedCount`, поэтому
queue нельзя объявить достижимой в этой сборке.
...
| registration | `0x004AD270` |
| primary vtable | `0x00490F60` |
| support vtable | `0x00490F84` |
...
Call ABI подтверждает общий контракт `(name, destination, callback, context)`.
Функция лениво создаёт resource/PCK resolver, получает canonical path и
проверяет префикс `host0:`. Однако обе ветки в shipped executable выполняют
одинаковую синхронную последовательность:
...
`0x001DA390` сначала проверяет `queuedCount`. При ненулевом состоянии он
обслуживает current record, использует watchdog `0x14` update-вызовов, пишет
строки:
...
Полный scan executable по offsets `+0x3794/+0x3798/+0x379C` нашёл только:
...
Поэтому поля названы analytical и queue не переносится в `Code/` как рабочая
функциональность.
