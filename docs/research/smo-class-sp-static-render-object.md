...
Это исправляет прежнюю ошибку writer-а: `Matrix4x4.Invert()` создавал внешне
валидный, но несовместимый с движком field 2 для любого authored scale. Теперь
writer воспроизводит transpose-basis convention и меняет обе матрицы одной
транзакцией.
...
engine inverse, разрешённую связь с `spModel` и корректную inline-топологию.
