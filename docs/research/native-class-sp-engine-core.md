# `spEngineCore`: общий bootstrap и владелец manager graph
...
| Поле | PC | PS2 |
|---|---:|---:|
| Class ID | `0x0E9F6B8C` | `0x0E9F6B8C` |
| Base | `spBaseObject / 0x415352A1` | `spBaseObject / 0x415352A1` |
| Registration | `0x0075DBA8` | `0x004A7C60` |
| Initializer | `0x006D26F0` | `0x004807B0` |
| Getter | `0x0041C5D0` | `0x001313E0` |
| Factory | `0x0041CB30` | `0x001335C0` |
...
PS2 constructor публикует complete object по GP-relative `-0x4920`, то есть в
`0x0049F850`. Destructor `0x00133270` и secondary deleting destructor
`0x00133490` очищают этот адрес. PC destructor `0x0041C500` очищает
`0x00755274`; deleting wrapper находится в `0x0041C640`. Обе реализации делают
очистку без проверки прежнего значения, что сохранено в portable lifetime.
...
- `Sparkplug/Code/Sparkplug/spEngineCore.h/.cpp`;
- отдельный target `SparkplugEngine`, зависящий от `SparkBase`;
- точные PC/PS2 evidence layouts и массивы всех 18 targets;
- unit test identity/factory/singleton, short-circuit четырёх стадий,
  callback dispatch, shutdown state и platform offset difference.

Изолированная Windows x64 сборка проходит `SparkBaseTests` и
`SparkplugEngineTests` (2/2). Тесты не запускают игру и не касаются её файлов.
