...
С 8 сентября текущие задачи выбираются по [функциям инструментов](tool-driven-research-scope.md).
Ниже сохраняется полный backlog неизвестного в движке. Его наличие не требует
дорабатывать незатронутые методы до выпуска проверенной команды приложения.
Default workbench queue показывает только активные срезы; отложенные доступны
через `--include-deferred`.
