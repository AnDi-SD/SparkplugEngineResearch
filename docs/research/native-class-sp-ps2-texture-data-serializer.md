...
веткой. Уточнение по original PS2 `00176CF0` от 10 сентября 2026: поле 6
меняет текущее слово выбора payload. `00176DB4/BC` сначала сохраняет manager
`+0x10` в stack `+0x88`; `00176E1C` читает field6 в тот же slot, а
`00176E58/E60` и `00176EEC/EF0` используют это слово с mask `0x08`.
Прежнее утверждение, что field6 не влияет на решение reader, было ошибочным.
...
При каждом поле `0/1` решение принимается по текущему слову. При неизменном
значении выбирается одна representation; повторное или переставленное field6
может изменить выбор для следующих полей. Проверка только общего source shape
или fallback «сначала PC, затем PS2» не воспроизводит этот dispatch.
...
слой не реализует полный PS2 runtime reader/writer. С 10 сентября отдельный
[общий metadata inspector](tool-ps2-texture-native-inspection-2026-09-10.md)
читает native section, raw scalars, palette и mip extents без создания target,
attachment, swizzle или GPU. Его bounds/caps — явные host restrictions.
