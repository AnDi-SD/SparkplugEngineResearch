# `spPS2App`: PS2 application shell
...
| primary vtable | `0x00491030` |
| support vtable | `0x00491054` |
...
Поскольку secondary callback получает pointer support-subobject, наблюдаемая
запись возле `+0x18` в его адресном пространстве не должна автоматически
переноситься на complete-object offset. Portable код выражает доказанный
результат через protected state seam общего `spApp`, не закрепляя неверную ABI
арифметику.
...
- inferred `Sparkplug/Code/SparkplugPS2/spPS2App.h/.cpp`;
- exact layout/addresses в `Sparkplug/Analysis/PS2/SparkBaseAbi.h`;
- конечный test subclass проверяет три вызова Update (`true, true, false`),
  termination state, false result и null-clone contract.
