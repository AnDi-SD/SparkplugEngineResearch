...
Отсутствие поля сохраняется decoder-ом как `null`. Старое межплатформенное
сравнение нормализовало его к нулю только как аналитическое допущение. Нативный
PS2 constructor `spModel` (`0x0015AA60`) записывает в runtime field `+0x54`
значение `3`, поэтому «отсутствует = эффективный 0» опровергнуто как общее
runtime-правило. PC constructor защищён `.rld`; writer не должен
материализовать field 1 до отдельной проверки обеих платформ.
