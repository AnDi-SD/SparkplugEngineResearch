...
Дополнение 7 сентября: [CP101](native-pc-whole-file-profile.md) в отдельном свежем
профиле `file` подтверждает whole `422B50` для `bflower`, `barrel`, `bw` и записи
C++ `barrel`, а также двух Node SMO. Ниже сохранены условия исторического
micro checkpoint; его остановки больше не описывают текущее покрытие этой выборки.
