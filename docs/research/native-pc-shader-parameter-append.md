# PC shader parameter production boundary — CP49

PC EXE SHA256 `3F022480BF55045DA4BF692E4BC8862ED38FC024E8A964A558FBDFDF646DFC4F`.
Actual4AF940 resolves to45A6C0; the complete method accepts a44-byte
descriptor **by value** and returns with ret2C. It calls actual4AE660 on
the leading name, replaces the descriptor type+20, appends via4AF850 to
shader+38 vector and then adds incoming count+28 to shader+34.
...
`pc-shader-parameter-append`:3/3 exact37 native, all22 names plus4 unknown
variants, overlapping starts, zero counts and wrap. Source35/35,
build76/76,CTest51/51 in50.77s. Report:
`local-data/results/bounded-native-runs/20260907T035505458474Z-pc-shader-parameter-append.json`.
Max6293instructions/3984arena; original PC shader and vector lifetime
fully freed. Preliminary scout entered resolved45A6C0 directly; final three
profiles use the actual protected4AF940 entry and assert the internal chain.
No cap increase, OS/GPU forwarding or original asset modification.

## Upstream static evidence, not executed compiler proof
...
pipeline. See [scope correction](native-pc-shader-template-frontier.md) for
