...
Checkpoint17, 6 сентября2026. Pristine PC SHA
`3F022480BF55045DA4BF692E4BC8862ED38FC024E8A964A558FBDFDF646DFC4F`.
Оригинальные инструкции исполнялись CPU-only, без OS/GPU/игры: 100k/2sec
на вызов,30sec на отдельный child. Whole Visibility constructor остаётся
открыт: fixtures дают явно описанный borrowed manager, не заменяют ctor успехом.

## Identity, имена и исходники

| Class | RTTI/base | Factory / exact size | Primary |
|---|---|---|---|
| `spZonePortal` |6523AC37→Named44DE07FD|481370 /38|6EBB10,8 slots|
| `spZonePortalNode` |ABB5AB2C→Node695C0F65|481810 /C4|6EBB44,14 slots|
...
at44E6FE. Two entries are the stock corpus invariant, not a container limit.
...
PartitionNode's portal64 vector owns intrusive references (prior checkpoint);
PortalNode's vector does not. Tests explicitly supply decoded graph relations
and matching owner references; this is not an implemented full serializer.
...
The **camera-near-portal-plane** route46CAAE..46CD5E separately tests projected
edge orientation, then calls **45E870** plane-vector copy at46CC77. Its whole
path is **not executed** here: previous Octree checkpoint already independently
hit the unchanged100k cap in that protected copy. Offset-adjusted recursion,
NaN/large polygons and complete491AA0 classification/alias contracts remain
open. DebugDraw481270 is only statically mapped (consecutive+closing edges to
DebugManager41DFA0), not a completed portable renderer.

## Reproduction and remaining work

```powershell
python research/probe_pc_zone_portal_runtime.py
python research/inspect_pc_zone_portal_runtime.py
python research/compare_pc_zone_portal_math.py
```
...
Next: logical continuation **491AA0 polygon clipping /scratch lifetime**,
not another unrelated easy class. Track unnamed helper identities and protected
near-plane copy/full Visibility ctor gaps alongside new evidence.

## Уточнения последующих checkpoints

[Clipping checkpoint](native-pc-polygon-clipping.md) уже проверил classification,
alias и ring lifetime491AA0 на описанных входах; историческая формулировка
выше относится к моменту этого portal checkpoint. Неизвестные helper names,
metadata20/24/28, гарантии размера входных полигонов и failure paths остаются.

В [цикле до12:00](native-pc-visibility-plane-storage.md) независимо подтверждён
46ADC0: near-plane caller46CC90 передаёт1 и **включает все унаследованные
плоскости**, пересчитывая activeCount. Это статическая caller связь плюс
исполнение отдельного helper, не прохождение полного near-plane пути:
предшествующий45E870 всё ещё неизвестен. Доступный45E530 copy-constructor
не является заменой этому assignment.
