...
`spFog` имеет Class ID `0x7AC95AEC`, напрямую наследует `spBaseObject` и
...
После `spBaseObject` лежат padding/unknown word `+0x10`, type `+0x14`, ARGB
...
base-copy, не копирующий fog payload. Portable реализация сохраняет этот
...
Открыты exact original paths/method spelling, смысл padding `+0x10`, детали
