...
Общие исходники: [spRenderer](../../../Sparkplug/Code/Sparkplug/spRenderer.h).
...
PC body распаковывает подтверждённые draw-поля `spDXMesh` по `+0x44..+0x84` и передаёт их в `0x004BC4A0`. Однако открытый downstream wrapper `0x004BC290` достоверно достигает D3D9 declaration/FVF, stream/index и `DrawPrimitive`/`DrawIndexedPrimitive` endpoints.
