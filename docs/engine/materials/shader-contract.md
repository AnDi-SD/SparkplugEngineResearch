...
Общая [spFixedShaderLighting.h](../../../Sparkplug/Analysis/PC/spFixedShaderLighting.h)
реализует diffuse-ветви directional, point и spot без specular и режимы цвета
0–7. Это семантическая реконструкция выражений ресурса; она не является найденным
CPU-методом игры. `ShadeFixedDiffuseForAnalysis` получает готовую `PosView`
в виде float4 и линейно преобразованную view-нормаль, которую нормализует.
`ShadeFixedDirectionalForAnalysis` сохраняет прежний вход из трёх view-регистров
и использует тот же расчёт освещения и цвета. Specular в этот интерфейс не входит.

Нормаль после skinning умножается на линейную часть трёх загруженных
float4-регистров `view_matrix`, затем нормализуется в трёх измерениях.
Направление света не нормализуется повторно. При положительном
`dot(Nview, -LightDir[i].xyz)` к накопленному RGB прибавляется произведение
этого dot и `LightMatDiff[i].rgb`. Alpha выбирается после освещения согласно
ColorMode. Четвёртые компоненты направления и diffuse света здесь не участвуют.

Результат общего интерфейса содержит единичную view-нормаль и float4-цвет
до упаковки COLOR0. RGB может превышать 1 либо быть отрицательным:
аналитический интерфейс не вводит насыщение внутри shader-выражения.
Преобразование в packed ARGB, интерполяция и последующие texture stages —
отдельные границы. При неизвестном режиме или типе света, числе источников
больше восьми, неконечном вычисляемом результате или нулевой нормали интерфейс
отказывает без изменения выхода. Локальные ветви также отказывают при нулевой
длине направления, используемом нулевом знаменателе attenuation или cone.
Это проверки аналитического интерфейса, а не восстановленные проверки шейдера.
Обычный `GetMaterial` не заменяет фактические float-константы активного VS.
...
влияет на результат; при неположительном dot не применяются ни diffuse, ни
attenuation. Коэффициенты attenuation.z/w не участвуют. Point со specular
этой attenuation не использует.

В point и spot сначала вычисляется `normalize(LightPos - PosView)` в четырёх
измерениях и лишь затем берётся xyz. Поэтому различие однородных компонент
позиций влияет на направление, хотя геометрическое расстояние для attenuation
по-прежнему равно длине xyz-разности. Повторно нормализовать полученный xyz нельзя.

Для spot cone равен `clamp((dot(L,-LightDir.xyz)-outer)/(inner-outer),0,1)`,
где L — это xyz четырёхмерно нормализованной разности. Diffuse без specular
прибавляется как `cone * LightMatDiff.rgb * dot(Nview,L)`: отрицательный dot
сохраняется, а attenuation по расстоянию отсутствует. Направление LightDir
берётся из shader-константы без дополнительной нормализации.

Все включённые UV-выходы начинаются с UV0 и собственного UVTransform[i].
Перед преобразованием исходный UV.z заменяется единицей. `float3x3` хранится
по столбцам: для трёх загруженных регистров C0/C1/C2 результат равен
`u*C0.xyz + v*C1.xyz + C2.xyz`. Четвёртые компоненты регистров не используются;
на третий компонент результата шейдер не делит. Порядок регистров отличается
от формулы affine palette Skin. Общий [расчёт UV](../../../Sparkplug/Analysis/PC/spFixedShaderSkinning.h)
принимает именно загруженные регистры; native uploader описан в
[контракте параметров](shader-constants.md).
