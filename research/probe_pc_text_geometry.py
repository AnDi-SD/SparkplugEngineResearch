...
def guest(folder,mode='normal'):
 ...
 if mode=='custom':
  ...
  f=TextureDeviceFixture()
 else:f=PCWriteBytesFixture()
 p=f.p
 ...
 try:
  ...
  if mode=='custom':p.put_uint(renderer+0xc9e8,f.device)
  font=call(0x462ec0);serializer=call(0x442500);manager=call(0x4c35a0);p.put_uint(0x755270,manager)
  atlas=0;custom_manager=0;custom_material=0;old_atlas=0;custom_holder=0
  if mode=='custom':
   atlas=call(0x4ab520);old_atlas=call(0x4ab520)
   def resolve(machine):
    expected=machine.uint(machine.reg('ESP')+4)
    check(machine.uint(machine.reg('ESP')+8)==f.stream,'declared reference stream')
    identity=struct.unpack_from('<I',f.data,f.position)[0]
    target=atlas if identity==0x2345 and expected==0x2f281e13 else custom_material if identity==0x3456 and expected==0x5c0314c5 else 0
    check(bool(target),'declared actual Font atlas/custom material reference')
    f.position+=8;machine.fixture_return(12,eax=target)
  ...
  data=field(0,(word(0x2345)+word(0) if mode=='custom' else word(0))+word(12)+word(3)+glyphs)+b'\0';f.data=data;f.position=0
  ...
  if mode=='custom':
   cases=[];custom_manager=call(0x4c35a0)
   check(call(0x41e8b0,this=custom_manager)&255,'actual second material producer')
   custom_material=p.uint(custom_manager+0x34);custom_pass=p.uint(custom_material+0x4c)
   custom_holder=p.uint(p.uint(custom_pass+0x18)+0x10)
   call(0x41e870,this=custom_holder,args=(old_atlas,))
   check(p.uint(custom_holder+0x34)==old_atlas and p.uint(old_atlas+8)&65535==1,'actual custom material initially owns different atlas')
   # A second original constructor publishes itself as the singleton. Select
   # the explicit primary renderer fixture again before invoking Text layout.
   p.put_uint(0x755270,manager)
  ...
  if mode in ('normal','custom'):
   ...
   for alignment in (0,1,2,7):
    data=(field(0,word(0x3456)+word(0)) if mode=='custom' else b'')+b'\0'+field(0,string(b'AB\0'))+field(1,word(0x7f123456))+field(3,word(alignment))+b'\0'
    ...
    if mode=='custom':
     check(p.uint(text+0x20)==custom_material and p.uint(custom_material+8)&65535==3,'actual Text owns custom material, temporary manager owner restored')
     check(p.uint(custom_holder+0x34)==atlas and old_atlas in f.freed and p.uint(atlas+8)&65535==2,'original replaces old atlas and retains actual Font image')
     check(next(e for e in events if e['kind']=='setup')['material']==custom_material,'backend sees selected actual Text material')
  ...
  if custom_manager:call(p.uint(p.uint(custom_manager)),this=custom_manager,args=(1,))
  ...
  for address in ((0x75db78,0x75526c,0x755264) if mode=='custom' else (0x75526c,0x755264)):
   obj=p.uint(address)
  ...
  if mode=='custom':
   check(f.device_refs==1 and len(f.device_events)==4,'actual atlas owners release both borrowed device references')
   report['customMaterial']=dict(id=f'{custom_material:08X}',atlas=f'{atlas:08X}',oldAtlas=f'{old_atlas:08X}',deviceEvents=f.device_events)
 ...
 if mode=='custom':
  report['remainingAllocations']=[dict(address=f'{a:08X}',size=n,refs=p.uint(a+8)&65535,vtable=f'{p.uint(a):08X}',data=bytes(p.mu.mem_read(a,n)).hex()) for a,n in f.allocations.items() if a not in f.freed]
  report['deviceEvents']=f.device_events;report['deviceRefs']=f.device_refs
