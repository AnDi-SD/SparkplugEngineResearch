...
def guest(class_name,output,factory_profile='micro',context='none',family='ai-action',tracer='instruction',lifecycle_profile='micro',crt='none'):
    ...
    if family not in ('ai-action','character-state','character-state-machine','entity-direct','entity-core','entity-manager'):raise ValueError('Explicit reviewed family required')
