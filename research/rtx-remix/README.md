...
* `WINX_REMIX_ORTHOGRAPHIC_UI=1`: временное отключение Z-write для
  fixed-function ортографических проходов основного render target,
  чтобы Remix классифицировал их как UI. Состояние восстанавливается
  после draw. Основная поверхность запоминается при CreateDevice/Reset.

`Original` выключает все эти адаптации. `Raster` включает первые две;
`RTX` включает все три. FVF-нормализация и чтение текстур оставлены только
...
изменение камеры, HUD и открытие дневника. Остаются чрезмерно оранжевое
освещение, отличия прозрачных/проекционных эффектов, искажения фона меню
и масштабирование интерфейса после перехода на 1280×960 в исходном окне.
...
powershell -NoProfile -ExecutionPolicy Bypass -File research/rtx-remix/Start-Probe.ps1 -Name fresh-test -ExplicitMipLevels -ResubmitTextures -Raytracing -OrthographicUi
```
