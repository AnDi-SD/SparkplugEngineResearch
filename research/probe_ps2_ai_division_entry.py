"""PS2 DIVU continuations; original PC results remain sealed and are not rerun."""
...
FOLDER=ROOT/'local-data/results/native-cycle-20260911-0730/ps2-unsigned-division'
...
LIMITS={'MinotaurAttack':0x25f358,'MinotaurDefend':Ps2ScalarPrefix.RETURN,'MosquitoAttack':0x2606a8}
def sha(b):return hashlib.sha256(b).hexdigest().upper()


def execute(c,k):
    assert k=='ps2' and c['name'] in ('MinotaurDefend','MosquitoAttack')
    pc=False;name=c['name'];entry=int(CATALOG[name][k]['slots'][10],16);outputs=[];calls=[]
    if pc:
        ...
        p=Ps2ScalarPrefix([(entry,0x200),(0x36ebc0,0x180),(0x14e640,0x28),(0x108350,0x1c),(0x108e30,0x68)],profile='integer-squares',stack_window=(0x22000000,4096),upper64=UPPER,unsigned_division=True)
    ...
    selector=not c['target'] and name=='MinotaurDefend'
    stop=(0x591c80 if pc else 0x225df0) if selector else None if pc else LIMITS[name]
    returns={0x5b8fc6,0x5b8a86,0x5b4bbc,0x5b4bf8} if pc else {0x25f300,0x25fd24,0x260618,0x260648}
    ...
    if pc:
        ...
        p.reg('A0',obj);p.reg('A1',c['argument']);saved={n:p.reg(n) for n in ('SP','RA','S0','S1','S2','S3','S4','F20','F21','F22','F23')};original=[read(a,n) for a,n in p.ranges];result=p.run(entry,[stop],count=6000,timeout_us=500000);assert original==[read(a,n) for a,n in p.ranges]
        result.update(unsupportedWord=f'{p.uint(stop):08X}' if name=='MosquitoAttack' else None,boundaryRegisters={n:f'{p.reg(n):016X}' for n in ('V0','S0','S1','F0','F1','F2','F3')})
    for hook in hooks:u.hook_del(hook)
    count=0 if name=='MinotaurAttack' and not c['target'] else 2 if name=='MosquitoAttack' else 1
    ...
    if name=='MinotaurDefend':ew(obj+0x3b4,c['now']+3000+outputs[0]%1501);ew(obj+0x3b8,cmd)
    else:ew(obj+0x3b8,c['now']+500+outputs[0]%501);ef(obj+0x3bc,-c['amplitude'])
    if stop==p.RETURN:
        assert {n:p.reg(n) for n in saved}==saved
        assert p.stack_extension.upper64==list(UPPER)
    assert len(result['unsignedDivisionObservations'])==2 and all(e['verified'] for e in result['unsignedDivisionObservations'])
    prior=ROOT/'local-data/results/native-cycle-20260911-0730/ai-random-entry'
    matched=[]
    for filename in ('random-pilot-run1.json','random-batch-run1.json'):
        path=prior/filename
        for row in json.loads(path.read_text())['cases']:
            if row['platform']=='pc' and row['input']==c:matched.append((path,row))
    assert len(matched)==1 and matched[0][1]['status']=='passed' and matched[0][1]['rngOutputs']==outputs
    result.update(archivedPcResult=dict(path=matched[0][0].relative_to(ROOT).as_posix(),sha256=sha(matched[0][0].read_bytes())),
        crossPlatformScope='Same selected input and original RNG outputs; new PS2 field guards use the deadline/negation formulas proven by archived PC guard checks. PC was not rerun; whole-object hashes are platform-specific.',
        observedDeadline=p.uint(obj+(0x3b4 if name=='MinotaurDefend' else 0x3b8)),
        observedAmplitudeBits=f'{p.uint(obj+0x3bc):08X}' if name=='MosquitoAttack' else None,
        preservedCalleeRegisters=saved if stop==p.RETURN else None)
    consumer=None
    if selector:
    ...
    result.update(boundary='selector' if selector else None if stop==p.RETURN else 'unsupported-arithmetic',consumer=consumer,rngOutputs=outputs,rngInputIndex=c['index'],rngOutputIndex=p.uint(index),nativeStateSourceSha256=sha(STATE.read_bytes()),arraySha256=sha(array_bytes),guardedBytes=0x6000,beforeSha256=sha(before),afterSha256=sha(after),expectedAfterSha256=sha(bytes(expected)))
    return result
...
def guest(output,selection):
    ...
    report=dict(kind='original-ps2-ai-division-entry',inputs=EXPECTED,sourceSha256=sha(Path(__file__).read_bytes()),selectionSha256=sha(path.read_bytes()),cases=[])
    for c in cases:
        for k in ('ps2',):
