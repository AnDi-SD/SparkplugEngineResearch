...
def main(mode,policy):
 ...
 result=subprocess.run([str(binary),'--write',mode,policy],input='\n'.join(native[2:4])+'\n',capture_output=True,text=True,timeout=10)
 if result.returncode:raise AssertionError(result.stderr)
 source=json.loads(result.stdout)
 if source!=native:raise AssertionError((native,source))
 print(f'PASS exact whole original/source DX mesh writer {mode}/{policy}; source backend reread completed');return 0
