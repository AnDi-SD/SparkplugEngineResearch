...
[Обязательные основы разработки](docs/engine/software-development-rules.md):
**наши программы → восстановленные исходники → игра; игра всегда права**.
Игровая логика используется из общих восстановленных классов. Платформенный
слой, включая renderer на OpenGL/Vulkan, реализуется нами под современные системы.
...
выполняются существующим восстановленным кодом. Самостоятельный разбор SMO ещё
предстоит заменить общими восстановленными классами; графический backend остаётся
нашей реализацией. Это промежуточная проверочная сборка: переработка Viewer
продолжается, интерфейс рассматривается отдельно.

[Аудит всего кода Viewer](docs/research/viewer-core-audit-2026-09-08.md): удалён
второй C# SAN decoder/sampler, общие операции полей/матриц подключены к Sparkplug,
запись и редактирование SMO вынесены из зависимостей Viewer. Оставшиеся SMO-декодеры,
bind-поза и эвристики перечислены отдельно: полного отсутствия смысловых дублей
пока нет. Все программы на общем Core теперь используют `SparkplugViewerNative.dll`;
для сборки рабочего дерева нужны Windows x64, полный workspace и Visual Studio
C++/CMake. Состав зависимостей будущих пакетов обновлён, новый релиз не собирался.
